...
aws s3 sync --acl public-read --profile newsapps --exclude '*.*' --include '*.svg' --cache-control 'max-age=604800' dist s3://$APP_S3_BUCKET/$PROJECT_SLUG/

echo "Syncing *.json files to S3..."
aws s3 sync --acl public-read --profile newsapps --exclude '*.*' --include '*.json' --cache-control 'max-age=3600' dist s3://$APP_S3_BUCKET/$PROJECT_SLUG/
